...
// fetch attempts for monthly tests
$sql = "
    SELECT
        at.attempt_id,
        a.user_roll,
    ...
    WHERE LOWER(t.test_type) = 'monthly'
...
foreach ($rows as $r) {
    ...
    if (!isset($monthlyTmp[$date])) $monthlyTmp[$date] = [];
    if (!isset($monthlyTmp[$date][$roll])) {
        $monthlyTmp[$date][$roll] = ['roll_no'=>$roll,'full_name'=>$name,'class'=>$cls,'score_sum'=>0,'total_sum'=>0];
    }
    $monthlyTmp[$date][$roll]['score_sum'] += $score;
    $monthlyTmp[$date][$roll]['total_sum'] += $total;
}

$monthlyGrouped = [];
foreach ($monthlyTmp as $date => $byRoll) {
    $items = array_values($byRoll);
    usort($items, function($a,$b){ return strcasecmp($a['roll_no'],$b['roll_no']); });
    $monthlyGrouped[$date] = ['label'=>$date,'items'=>$items];
}
uksort($monthlyGrouped, function($a,$b){ return strcmp($b,$a); });
?>
<!doctype html>
<html>
<head>
...
<main class="container">
    ...
    <div id="report"><br>
        <?php if (empty($monthlyGrouped)): ?>
            <div class="muted">No monthly attempts found.</div>
        <?php else: ?>
            <?php foreach ($monthlyGrouped as $date => $grp): ?>
...
<script>
function printReport(){ const t=document.title; document.title='monthly_test_report'; window.print(); document.title=t; }
</script>
</body>
</html>
